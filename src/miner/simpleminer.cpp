...
namespace mining
{
 ...
 // static const int attempts_per_loop = 5000;
	const command_line::arg_descriptor<uint32_t> arg_platform = { "platform", "Specify OpenCL platform number (starting at 0)", 0, true };
	const command_line::arg_descriptor<uint32_t> arg_device = { "device", "Specify OpenCL device number (starting at 0)", 0, true };

	static const int attempts_per_loop = 64 * 1024 * 4;
	...
	void simpleminer::init_options(boost::program_options::options_description& desc)
	{
		...
		command_line::add_arg(desc, arg_platform);
		command_line::add_arg(desc, arg_device);
  ...
  bool simpleminer::init_scratchpad()
  {
    ...
    m_last_scratchpad_store_time = 0;
		...
		m_platform = 0;
		if(command_line::has_arg(vm, arg_platform))
			m_platform = command_line::get_arg(vm, arg_platform);
		m_device = 0;
		if(command_line::has_arg(vm, arg_device))
			m_device = command_line::get_arg(vm, arg_device);
    if(command_line::has_arg(vm, arg_scratchpad_url))
    {
      m_scratchpad_url = command_line::get_arg(vm, arg_scratchpad_url);
		...
		pOCL_Device = new OCL_Device(m_platform, m_device, attempts_per_loop, 0, "wild_keccak.cl");
		if (!pOCL_Device->isOK())
			exit(-1);
	...
	void simpleminer::worker_thread(uint64_t start_nonce, uint32_t nonce_offset, std::atomic<uint32_t> *result, std::atomic<bool> *do_reset, std::atomic<bool> *done) {
		...
		while (!*do_reset) {
			...
			LOG_PRINT_L3("Blob size " << blob.size() << ", scratchpad " << m_scratchpad.size() << ", start " << start_nonce << ", offset " << nonce_offset);
			
			char* data = new char[blob.size()];
			memcpy(data, blob.data(), blob.size());
			pOCL_Device->CopyBufferToDevice(data, 0, blob.size());	
#define OUTPUT_SIZE 16
			...
			for (uint64_t i = 0; i < output[OUTPUT_SIZE-1]; i++) {
				...
				LOG_PRINT_L2("GPU " << (nounce));

				(*reinterpret_cast<uint64_t*>(&blob[1])) = nounce;
...
//				LOG_PRINT_L2("CPU " << h);
				...
				{
					LOG_PRINT_L2("CPU " << (nounce));
					(*result) = (uint32_t)(nounce - start_nonce);
					(*done) = true;
					(*do_reset) = true;
...
//			nonce_offset += ((m_threads_total-1) * attempts_per_loop);
			nonce_offset += (m_threads_total * attempts_per_loop);
		}
	}
